...
export const gradientShift: SketchDefinition = {
  ...
  create: (): SketchInstance => {
    let p!: p5;
    let bgImgShader, palette, sketch, u_hueRotation, u_saturation, u_brightness, globalNoiseFunc, bgColor, bgImg;
    let bufferShader, renderShader, smearShader, sim0, sim1, col0, col1;
    ...
    class ShiftingGradient {
      constructor() {
        this.type = p.random([0, 1]);
        this.originalColors = palette;
        this.colors = [...this.originalColors];
        this.coords = new Coordinates(this.type);
        this.colorUpdateDuration = p.random(durations);
        this.stops = [];
        for (let i = 0; i <= this.colors.length * 2; i++) {
          this.stops.push(i / this.colors.length / 2);
        }
        this.img = p.createGraphics(p.width, p.height); // TODO look into framebuffer
        this.step(0);
      }
    
      setup() {
        if (this.type === 0) {
          this.gradient = this.img.drawingContext.createLinearGradient(...this.coords.args);
        } else if (this.type === 1) {
          this.gradient = this.img.drawingContext.createRadialGradient(...this.coords.args);
        } else {
          this.gradient = this.img.drawingContext.createConicGradient(...this.coords.args);
        }
        this.stops.forEach((stop, i) => {
          this.gradient.addColorStop(stop, this.colors[i % this.colors.length]);
        });
      }
      ...
      shiftStops() {
        for (let i = 0; i < this.stops.length; i++) {
          ...
          while (check < this.stops[i - 1] || check > this.stops[i + 1] || check < 0 || check > 1) {
      ...
      updateColors(amt, offset) {
        ...
        for (let i = 0; i < this.originalColors.length; i++) {
          let ind = i + offset;
          if (ind < 0) {
            ind = this.originalColors.length - (abs(ind) % this.originalColors.length);
          }
      ...
      draw() {
        this.img.clear();
        this.img.drawingContext.fillStyle = this.gradient;
        this.img.drawingContext.fillRect(0, 0, p.width, p.height);
        return this.img;
      }
    ...
    class Coordinates { // TODO can this be simplified
      constructor(type) {
        const dim = p.min(p.width, p.height);
        this.type = type;
        this.positionDuration = p.random(durations);
        this.xDir = p.random([-1, 1]);
        this.yDir = p.random([-1, 1]);
        this.positionOffset = p.random(9999);
        this.maxXShiftFactor = p.random();
        this.maxYShiftFactor = p.random();
        this.maxXShift = this.maxXShiftFactor * dim;
        this.maxYShift = this.maxYShiftFactor * dim;

        this.radiusDuration = p.random(durations);
        this.radiusDir = p.random([-1, 1]);
        this.radiusOffset = p.random(9999);
        this.maxRShiftFactor = p.random();
        this.maxRShift = this.maxRShiftFactor * (dim / 2);

        this.angleDuration = p.random(durations);
        this.angleDir = p.random([-1, 1]);
        this.angleOffset = p.random(9999)
        this.maxAngleShift = p.random(2*p.PI);

        this.startAngle = p.random(2*p.PI);
        if (this.type === 0) {
      ...
      setArgs(xShift = 0, yShift = 0, rShift = 0, angleShift = 0) {
        if (this.type === 0) {
          this.args = [this.x0 + xShift, this.y0 + yShift, this.x1 + xShift, this.y1 + yShift];
        } else if (this.type === 1) {
          this.args = [this.x0 + xShift, this.y0 + yShift, p.max(this.r0 + rShift, 0), this.x0 + xShift, this.y0 + yShift, p.max(this.r1 + rShift, 0)];
        } else {
          this.args = [this.startAngle + angleShift, this.x0 + xShift, this.y0 + yShift];
        }
      }
      ...
      step(elapsedTime) {
        ...
        const angleShift = p.sin((elapsedTime + this.angleOffset) / this.angleDuration) * this.maxAngleShift * this.angleDir;
        this.setArgs(xShift, yShift, rShift, angleShift);
      }
    ...
    // Utils
    const randomAttribute = function(obj) {
      const keys = Object.keys(obj);
      const val = Math.floor(keys.length * p.random());
      return obj[keys[val]];
    }
    
    function getNoiseFunc() {
      const terraces = p.floor(p.random(3, 10));
      const noiseOptions = {
        classic: (xOff, yOff, zOff) => {
          return p.noise(xOff, yOff, zOff)
        },
        terraces: (xOff, yOff, zOff) => {
          return p.ceil(p.noise(xOff, yOff, zOff) * terraces)/terraces;
        },
        ridges: (xOff, yOff, zOff) => {
          function ridgeNoise(nx, ny, nz) {
            return 2 * (0.5 - p.abs(0.5 - p.noise(nx, ny, nz)));
          }
          return Math.pow(ridgeNoise(xOff, yOff, zOff), 5);
        },
        octaves: (xOff, yOff, zOff) => {
          const noiseVal = p.noise(xOff, yOff, zOff) + .5 * p.noise(2 * xOff, 2 * yOff, 2 * zOff) + .25 * p.noise(4 * xOff, 4 * yOff, 4 * zOff);
          return noiseVal/(1 + .5 + .25);
        },
        power: (xOff, yOff, zOff) => {
          const noiseVal = p.noise(xOff, yOff, zOff) + .5 * p.noise(2 * xOff, 2 * yOff, 2 * zOff) + .25 * p.noise(4 * xOff, 4 * yOff, 4 * zOff);
          return p.map(Math.cbrt((noiseVal/(1 + .5 + .25)) - .5), Math.cbrt(-.5), Math.cbrt(.5), 0, 1);
        },
        steps: (xOff, yOff, zOff) => {
          return Math.round(p.noise(xOff, yOff, zOff) * 30)/30;
        }
      };
      return randomAttribute(noiseOptions);
    }
    ...
    return {
      setup: (ctx) => {
        ...
        // renderShader = p.createShader(vertSrc, renderFragSrc);
      ...
      draw: () => {
        ...
        // renderShader.setUniform('u_data', sim0);
        // renderShader.setUniform('u_tex', bgImg);
        // renderShader.setUniform('u_resolution', [p.width, p.height]);
